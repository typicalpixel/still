...
- `standalone`: one server runs everything. Home servers, VPS, small teams.
- `controller`: owns the database, exposes the API, and orchestrates deploys across a multi-server fleet.
- `agent`: runs application workloads in a multi-server fleet and talks to the controller over Erlang distribution.
...
- `STILL_MODE`: `standalone` (controller + agent on one box; the
  default, and the right choice for a single server), `controller`, or
  `agent`.
- `STILL_CONTROLLER_NODE` (agents only): where
  this agent reaches the controller over Erlang distribution, e.g.
  `still@10.0.0.1`.
- `STILL_CONTROLLER_DOMAIN` (controller / standalone): the
  public hostname you reach Still on, e.g. `still.example.com`. It serves the
  LiveView dashboard at `/` and the JSON API under `/api` on the same origin. Point
...
- `STILL_CONTROLLER_TLS` (controller / standalone): does this server
...
- `STILL_NODE_HOST` (all modes): the address other Still
  ...
  necessarily public).
Then it writes a systemd unit, runs migrations, reconciles Caddy's base config, and prompts for a first admin user on fresh controller/standalone installs.
...
the per-app domains Still proxies. A request on any other host gets a
small "Still" page instead of the dashboard.

## Create the first admin

A fresh controller/standalone install has no users. On an interactive install the script runs `bin/bootstrap`, which prompts for an admin email and password so you finish ready to sign in. A piped or non-interactive install (`curl | sh`, Ansible) can't answer that prompt, so create the admin yourself with one of these paths:

- CLI: `sudo /opt/still/bin/bootstrap` prompts for email + password, creates the admin, and prints a one-time admin API key for immediate API use.
...
Returns `{"data": {"token": "...", "user": {...}}}` with a session token, so there's no separate login step. Bootstrap does not hand out an API key; mint one explicitly with that token (see [API keys](#api-keys)) when you need CI/CLI access. The endpoint is one-shot and returns 409 once any user exists.
...
The raw key (prefixed `still_...`) is returned once. Save it; it is never shown again. Use it the same way as a session token.
...
Skip this on standalone. The installer auto-registers the local host.
...
- Relative, default: `bin/hello start`. Still expands it to the
...
- Prefix runner: wrap the start command in a launcher that fetches secrets
  ...
  starts with `/`, Still passes it through verbatim, so include the slot path
  yourself:
  ...
  launcher works identically, for example [Infisical](https://infisical.com)
...
Two optional commands round out the unit. Both resolve the same way: relative paths become slot paths, and absolute paths pass through verbatim.
...
| `STILL_TARGET_SLOT` | `blue` or `green`; the slot this instance is |
| `STILL_NODE_HOST` | host the node advertises on |
| `STILL_RELEASE_VERSION` | the version being deployed |

If your release starts distributed Erlang, which is the `mix release` default and what
...
Still gives you the slot and leaves the node name to your release. Fold the slot into
...
That yields `app-blue@...` and `app-green@...`. The names are distinct, so both coexist during
the overlap.
...
| 9090 | Caddy (artifact server) | `STILL_INTERNAL_PORT` | reachable from agents on the same network as Erlang distribution |
...
Admins can read the live Caddy config from the API when traffic is not routing the way they expect:
...
To work on the dashboard without a real agent or a `mix release`, seed a faked standalone install with an admin login, a server, sample apps, and deploy history, all announced as connected. From an `iex -S mix phx.server` session:
...
If you run `:integration_root` tests without being root, the affected modules skip with a clear reason instead of failing. Same if a host executable is missing for any category.
...
Still leans hard on [Caddy](https://caddyserver.com/) for ingress and TLS, and we love it. We're not affiliated with, endorsed by, or sponsored by the Caddy project or [ZeroSSL](https://zerossl.com/). We drive Caddy's admin API, and Caddy remains its own project under its own license.
