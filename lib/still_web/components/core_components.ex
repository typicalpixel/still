defmodule StillWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: StillWeb.Gettext
  ...
  defp error(assigns) do
    ~H"""
    <p class="mt-1.5 flex gap-2 items-center text-[12px] text-rust-700 dark:text-rust-300">
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-end justify-between gap-6", "pb-4"]}>
      ...
      <div :if={@actions != []} class="flex flex-none items-center gap-2">{render_slot(@actions)}</div>
    </header>
    """
  end
        ...
        <tbody id={@id} phx-update={is_struct(@rows, Phoenix.LiveView.LiveStream) && "stream"}>
          ...
          >
            <td
              ...
              class={[
                "px-5 py-4 text-[13px] text-paper-700 dark:text-ink-100",
                @row_click && "hover:cursor-pointer"
              ]}
            >
              {render_slot(col, @row_item.(row))}
            </td>
            <td :if={@action != []} class="px-5 py-4 text-right">
  ...
  @doc "JS command: reveals the matched element with a transition."
  ...
  @doc "JS command: hides the matched element with a transition."
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(StillWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(StillWeb.Gettext, "errors", msg, opts)
    end
  end
