defmodule StillWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, StillWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to the signed-in path when already logged in", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.log_in_user(user)
    ...
    test "writes a cookie if remember_me was set in previous session", %{conn: conn, user: user} do
      ...
      # Already logged in with remember_me set: a fresh log in (without
      # explicitly setting remember_me) should set the cookie again.
      conn = UserAuth.log_in_user(conn, user, %{})
      assert %{value: signed_token, max_age: max_age} = conn.resp_cookies[@remember_me_cookie]
      assert signed_token != get_session(conn, :user_token)
      assert max_age == @remember_me_cookie_max_age
  ...
  describe "log_out_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      StillWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    ...
    test "reissues a new token after a few days and refreshes cookie", %{conn: conn, user: user} do
      ...
      override_token_inserted_at(token, DateTime.add(DateTime.utc_now(), -10, :day))
  ...
  describe "on_mount :mount_current_scope" do
    ...
    test "assigns nil to current_scope if there isn't a valid user_token", %{conn: conn} do
    ...
    test "assigns nil to current_scope if there isn't a user_token", %{conn: conn} do
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        StillWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
